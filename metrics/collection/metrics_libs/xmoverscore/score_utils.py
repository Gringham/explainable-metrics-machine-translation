    ...
    for batch_start in tqdm.tqdm(range(0, len(src), batch_size)):
        ...
        for i in range(batch_size):
            ...
            if drop_punctuation == True:
                ...
                # If they punctuation and subwords are dropped, we collapse the list in order to determine the words
                ...
                # Otherwise we will directly map tokens to scores and handle the mapping to words in the Explainer and
                # Evaluation Script
            ...
            # Here we find the token combinations that are used in n-gram mode (for 1 grams its just a list of tokens)
            ...
            # Using the emd with flow here, as we also tried to use the max across the flow matrix, which wasn't
            ...
            # Depending on the case we already dropped the beginning tokens or not. Hence we get the token scores differently
